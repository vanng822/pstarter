...
module.exports.statServer = function(port) {
		...
		if(msg.cmd) {
			switch(msg.cmd) {
				case cmd.requestCount:
					...
					if (msg.pid && statistics.workers[msg.pid]) {
						statistics.workers[msg.pid].count++;
					}
					break;
			}
		}
...
function workerOnfork(worker) {
	...
	statistics.workers[worker.process.pid] = {created: new Date(worker.created), count:0, isDead: false};
}
		...
		isRunning(pid, function(running) {
			...
			cluster.on('exit', function(worker, code, signal) {
				...
				if (statistics.workers[worker.process.pid]) {
					statistics.workers[worker.process.pid].isDead = true;
				}
