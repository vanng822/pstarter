...
if('MAX_REQUEST' in process.env) {
	MAX_REQUEST = parseInt(process.env['MAX_REQUEST']);
}
...
function workerOnfork(worker) {
	...
	worker.once('online', function() {
		console.info('Worker ' + worker.process.pid + ' is online at ' + (new Date()).toISOString());
	});
...
module.exports.startMaster = function(confFile, masterSettings, callback) {
	if(cluster.isMaster) {
		...
		if (MAX_REQUEST === null) {
			if (config.MAX_REQUEST) {
				MAX_REQUEST = parseInt(config.MAX_REQUEST);
			} else {
				MAX_REQUEST = 0;
			}
		}
		...
		isRunning(pid, function(running) {
			...
			cluster.on('exit', function(worker, code, signal) {
				...
				if(statistics.workers[worker.process.pid]) {
					...
					statistics.deadWorkers[worker.process.pid].exitedAfterDisconnect = worker.exitedAfterDisconnect;
...
var handleWatch = function(event, filename, exts) {
	...
	switch(event) {
		case 'change':
		case 'rename':
			if(util.isArray(exts) && exts.length > 0) {
...
var patrol = function(init) {
	var i, len, wids, timeout, worker, pid;
	if (init) {
		return setInterval(patrol, PATROL_POLL_TIMEOUT);
	}
	wids = Object.keys(cluster.workers);
	timeout = 0;
	forkWorkers(config.http.numWorkers - wids.length);
	if (MAX_REQUEST > 0) {
		for (i = 0, len = wids.length; i < len; i++) {
			worker = cluster.workers[wids[i]];
			pid = worker.process.pid;
			if (statistics.workers.hasOwnProperty(pid)) {
				if (statistics.workers[pid].count >= MAX_REQUEST) {
					gracefulWorker(worker, timeout);
					timeout += gracefulStep;
				}
			}
		}
